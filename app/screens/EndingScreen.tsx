...
export function EndingScreen({ navigation }: Props) {
  const ending = useGameStore((s) => s.ending);
  const runSummary = useGameStore((s) => s.runSummary);
  const startNewRun = useGameStore((s) => s.startNewRun);

  useEffect(() => {
    // Reached directly (e.g. a reload mid-navigation) with nothing to show.
    if (!ending || !runSummary) navigation.replace("Home");
  }, [ending, runSummary, navigation]);
  ...
  function handleShare() {
    ...
    // absent, so handleShare is only ever reachable (via onPress) when both
    // are set — TS just can't see that across the closure.
    Share.share({
      message:
        `${ending!.title}\n\n${ending!.description}\n\n` +
...
const styles = StyleSheet.create({
  ...
  content: { flex: 1, justifyContent: "center", gap: space(4) },
  eyebrow: {
    color: color.inkDim,
    fontSize: 11,
